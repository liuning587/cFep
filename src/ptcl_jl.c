...
/**
 ******************************************************************************
 * @brief   丢弃吉林规约组帧状态并释放缓冲
 * @param[in,out] pchk 报文检测上下文
 *
 * @return  None
 * @details 与国网帧结构类似，L 域异常时须 reset，避免死循环写 pbuf。
 ******************************************************************************
 */
static void
jl_chkfrm_reset(chkfrm_t *pchk)
{
    if (pchk->pbuf != NULL)
    {
        free(pchk->pbuf);
 ...
 ******************************************************************************
 * @brief   吉林规约报文检测
 * @details 防护：组帧超长、pbuf_pos>5+dlen、写入前检查 pbuf 与 cap。
        ...
        int rxLen)
{
    ...
    while (rxLen > 0)
    {
        ...
        {
            log_print(L_ERROR, "吉林规约组帧超长,已丢弃\n");
            jl_chkfrm_reset(pchk);
        }
        ...
        switch (pchk->frame_state)
        {
            ...
            case JL_FRAME_STATES_LEN1_1:
                pchk->frame_state = JL_FRAME_STATES_LEN1_2;
                pchk->dlen = ((*rxBuf & 0xFCu) >> 2u);
                break;

            case JL_FRAME_STATES_LEN1_2:
            ...
            case JL_FRAME_STATES_CONTROL:
                pchk->cs = *rxBuf;
                pchk->frame_state = JL_FRAME_STATES_A3;
                break;
            ...
            case JL_FRAME_STATES_LINK_USER_DATA:
                pchk->cs += *rxBuf;
                /* 与国网：L 过小则无法到达 CS，原逻辑会持续写 pbuf */
                if (pchk->pbuf_pos > (5u + pchk->dlen))
                {
                    log_print(L_DEBUG, "吉林规约 L 域与头部长度矛盾,丢弃\n");
                    jl_chkfrm_reset(pchk);
                    break;
                }
                if (pchk->pbuf_pos == (5u + pchk->dlen))
